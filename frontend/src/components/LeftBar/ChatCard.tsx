...
const ChatCard: React.FC<ChatCardProps> = ({ chat, hasUnread }) => {
    ...
    }, [user, chat.users, getUserInfo]);
        
    return (
        <button
            onClick={() => open(chat)}
            key={chat.id}
            className={`flex flex-row items-center hover:bg-gray-100 rounded-xl p-2 ${hasUnread ? 'bg-blue-100' : ''}`}
        >
            <div className="flex items-center justify-center h-8 w-8 bg-indigo-200 rounded-full overflow-hidden relative">
                {hasUnread && (
                    <span className="absolute top-0 right-0 bg-red-500 rounded-full h-3 w-3 text-white text-xs flex justify-center items-center">
                        •
                    </span>
                )}
                <img
                    src={`https://placehold.co/100x100/${getColorFromString(friend?.name || '')}/31343C?font=oswald&text=${friend?.name?.[0] || ''}`}
                    alt="Avatar"
