# app/routes/chat.py
#Handles routes related to one-on-one chat functionalities
#,including creating a chat, sending messages, and listing/receiving chats.
...
#This route creates a new one-to-one chat between two users.
...
def create_chat():
    ...
    #retrieves the sender's ID from the JWT token
    sender_id = get_jwt_identity()

    #Request Body: withEmail (the email of the user to chat with)
    user_email = data.get("withEmail")
    
    #queries the database of sender's ID from the JWT token to get the user object.
